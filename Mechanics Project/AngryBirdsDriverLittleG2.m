function [struct,realTime] =  AngryBirdsDriverLittleG2
...
%%%%
%DONT FORGET TO TAKE OUT THE POINTS AFTER THE PIG COLLISION EMILY!!!!!!!
%the points after 8 seconds...
%:)
%%%%

BirdData = textread('BirdData22.txt');
...
%solve the coupled ODEs
    ...
    %interpolated X values from the physical models
    ...
    xResMean = mean(xResiduals);
    yResiduals = interpYNew-interpYActual;
    yResMean = mean(yResiduals);
    
    NewResSum = xResMean + yResMean;
    
    if NewResSum < OldResSum
        OldResSum = NewResSum;
  ...
  figure
    hold on
        plot(interpTime,interpYActual,interpTime,interpYNew,interpTime,yResiduals);
